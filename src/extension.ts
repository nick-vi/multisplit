...
async function getFilesRecursively(uri: vscode.Uri, ignoreFilter: IgnorePatterns | null = null, workspaceRoot: string = ''): Promise<vscode.Uri[]> {
...
async function loadSplitIgnore(workspaceRoot: string): Promise<IgnorePatterns | null> {
    try {
...
export async function activate(context: vscode.ExtensionContext) {
    const outputChannel = vscode.window.createOutputChannel('Split View Extension');
    ...
    const openInSplitViewDisposable = vscode.commands.registerCommand('extension.openInSplitView', async (uriOrUris: vscode.Uri | vscode.Uri[], ...args: any[]) => {
        try {
            let uris: vscode.Uri[] = [];
            ...
            if (args.length > 0 && Array.isArray(args[0])) {
                try {
                    ...
                    if (selectedUris.length > 0 && typeof selectedUris[0].fsPath === 'string') {
                        uris = selectedUris.map((item: any) => vscode.Uri.file(item.fsPath));
                    }
                } catch (error) {
            ...
            for (const uri of uris) {
                ...
                if (stat.type === vscode.FileType.Directory) {
                    const files = await getFilesRecursively(uri, ignoreFilter, workspaceRoot);
                    ...
                    if (ignoreFilter && workspaceRoot) {
                        ...
                        if (!ignoreFilter.ignores(relativePath)) {
                            ...
                            log(`Ignoring file: ${uri.fsPath} (matched by .splitignore pattern)`);
                        }
                    } else {
                        allFiles.push(uri);
                    }
            ...
            // Limit to 16 files maximum
            const MAX_FILES = 16;
            let files = textFiles;

            if (files.length < 2) {
                vscode.window.showWarningMessage('Need at least 2 text files to open in split view.');
                return;
            }
            ...
            if (files.length > MAX_FILES) {
                ...
                vscode.window.showWarningMessage(`Limited to ${MAX_FILES} files. Use .splitignore to filter unwanted files.`);
            }
            ...
            await vscode.commands.executeCommand('vscode.setEditorLayout', {
                orientation: 0,
                groups: Array(rows).fill(null).map(() => ({
                    groups: Array(cols).fill(null).map(() => ({ size: 1 }))
                }))
            });
            ...
            vscode.window.showInformationMessage(`Opened ${files.length} file(s) in split view`);
        } catch (error) {
